...
// Model for SalesAgreement entity
    models.SalesAgreement = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.salesagreement/",
        idAttribute: 'idSalesAgreement',
        defaults: {
            commission: "",
            purchasePrice: "",
            realEstate: {idRealEstate: "", address: "", idUser: {idUser: "", firstName: "", lastName: ""}},
            buyer: {idUser: "", firstName: ""}
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idSalesAgreement');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.salesagreement/';
            }
    ...
    // Collection class for SalesAgreement entities
    models.SalesAgreementCollection = Backbone.Collection.extend({
        model: models.SalesAgreement,
        url: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.salesagreement/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.SalesAgreementCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-salesagreement-list-itemis template identifier for item
                        templateName: '#tpl-salesagreement-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.salesagreement = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.salesagreement,
                    // tpl-salesagreement-details is template identifier for chosen model element
                    templateName: '#tpl-salesagreement-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.SalesAgreement();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-salesagreement-details is a template identifier for chosen model element
                templateName: '#tpl-salesagreement-details',
        ...
        getData: function () {
            return {
                idSalesAgreement: $('#idSalesAgreement').val(),
