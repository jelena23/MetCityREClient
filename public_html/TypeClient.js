...
// Model for Type entity
    models.Type = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.type/",
        idAttribute: 'idType',
        defaults: {
            typeName: ""
        },
        toViewJson: function () {
            ...
            result.displayName = this.get('idType');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.type/';
            }
    ...
    // Collection class for Type entities
    models.TypeCollection = Backbone.Collection.extend({
        model: models.Type,
        url: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.type/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.TypeCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-type-list-itemis template identifier for item
                        templateName: '#tpl-type-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.type = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.type,
                    // tpl-type-details is template identifier for chosen model element
                    templateName: '#tpl-type-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Type();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-type-details is a template identifier for chosen model element
                templateName: '#tpl-type-details',
