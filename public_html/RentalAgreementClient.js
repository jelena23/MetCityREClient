...
// Model for RentalAgreement entity
    models.RentalAgreement = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.rentalagreement/",
        idAttribute: 'idRentalAgreement',
        defaults: {
            deposit: "",
            monthlyPrice: "",
            realEstate: {idRealEstate: "", address: "", idUser: {idUser: "", firstName: "", lastName: ""}},
            tenant: {idUser: "", firstName: ""}
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idRentalAgreement');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.rentalagreement/';
            }
    ...
    // Collection class for RentalAgreement entities
    models.RentalAgreementCollection = Backbone.Collection.extend({
        model: models.RentalAgreement,
        url: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.rentalagreement/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.RentalAgreementCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-rentalagreement-list-itemis template identifier for item
                        templateName: '#tpl-rentalagreement-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.rentalagreement = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.rentalagreement,
                    // tpl-rentalagreement-details is template identifier for chosen model element
                    templateName: '#tpl-rentalagreement-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.RentalAgreement();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-rentalagreement-details is a template identifier for chosen model element
                templateName: '#tpl-rentalagreement-details',
        ...
        getData: function () {
            return {
                idRentalAgreement: $('#idRentalAgreement').val(),
                deposit: $('#deposit').val(),
