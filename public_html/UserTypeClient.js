...
// Model for UserType entity
    models.UserType = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.usertype/",
        idAttribute: 'idUserType',
        defaults: {
            userTypeName: "",
            userTypeDescription: ""
        },
        toViewJson: function () {
            ...
            result.displayName = this.get('idUserType');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.usertype/';
            }
    ...
    // Collection class for UserType entities
    models.UserTypeCollection = Backbone.Collection.extend({
        model: models.UserType,
        url: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.usertype/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.UserTypeCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-usertype-list-itemis template identifier for item
                        templateName: '#tpl-usertype-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.usertype = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.usertype,
                    // tpl-usertype-details is template identifier for chosen model element
                    templateName: '#tpl-usertype-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.UserType();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-usertype-details is a template identifier for chosen model element
                templateName: '#tpl-usertype-details',
        ...
        getData: function () {
            return {
                idUserType: $('#idUserType').val(),
                userTypeName: $('#userTypeName').val(),
                userTypeDescription: $('#userTypeDescription').val()
            };
        }
    });
    new AppRouter();
