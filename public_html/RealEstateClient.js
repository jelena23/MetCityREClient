...
// Model for RealEstate entity
    models.RealEstate = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.realestate/",
        idAttribute: 'idRealEstate',
        defaults: {
            address: "",
            size: "",
            price: "",
            ...
            numOfBathrooms: "",
            heating: "",
            type: {idType: "", typeName: ""},
            municipality: {idMunicipality: "", municipalityName: ""},
            user: {idUser: "", firstName: "", lastName: ""},
            realEstateType: {idRealEstateType: "", realEstateType: ""}
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idRealEstate');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.realestate/';
            }
    ...
    // Collection class for RealEstate entities
    models.RealEstateCollection = Backbone.Collection.extend({
        model: models.RealEstate,
        url: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.realestate/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.RealEstateCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-realestate-list-itemis template identifier for item
                        templateName: '#tpl-realestate-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.realestate = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.realestate,
                    // tpl-realestate-details is template identifier for chosen model element
                    templateName: '#tpl-realestate-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.RealEstate();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-realestate-details is a template identifier for chosen model element
                templateName: '#tpl-realestate-details',
        ...
        getData: function () {
            return {
                idRealEstate: $('#idRealEstate').val(),
                address: $('#address').val(),
                size: $('#size').val(),
                price: $('#price').val(),
                ceilingHeight: $('#ceilingHeight').val(),
                numOfRooms: $('#numOfRooms').val(),
                floor: $('#floor').val(),
                numOfBathrooms: $('#numOfBathrooms').val(),
                heating: $('#heating').val()
            };
        }
    });
