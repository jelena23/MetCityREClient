...
// Model for City entity
    
    models.City = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.city/",
        idAttribute: 'idCity',
        defaults: {
            cityName: "",
            country: {idCountry: "", countryName: ""}
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idCity');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.city/';
            }
    ...
    // Collection class for City entities
    models.CityCollection = Backbone.Collection.extend({
        model: models.City,
        url: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.city/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CityCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-city-list-itemis template identifier for item
                        templateName: '#tpl-city-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.city = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.city,
                    // tpl-city-details is template identifier for chosen model element
                    templateName: '#tpl-city-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.City();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-city-details is a template identifier for chosen model element
                templateName: '#tpl-city-details',
...
//                idCountry: $('#idCountry.idCountry').val(),
//                countryName: $('#idCountry.countryName').val()
            };
        }
    });
    new AppRouter();
