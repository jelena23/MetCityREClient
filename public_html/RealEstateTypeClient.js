...
// Model for RealEstateType entity
    models.RealEstateType = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.realestatetype/",
        idAttribute: 'idRealEstateType',
        defaults: {
            realEstateTypeName: "",
            realEstateTypeDescription: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('idRealEstateType');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.realestatetype/';
            }
    ...
    // Collection class for RealEstateType entities
    models.RealEstateTypeCollection = Backbone.Collection.extend({
        model: models.RealEstateType,
        url: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.realestatetype/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.RealEstateTypeCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-realestatetype-list-itemis template identifier for item
                        templateName: '#tpl-realestatetype-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.realestatetype = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.realestatetype,
                    // tpl-realestatetype-details is template identifier for chosen model element
                    templateName: '#tpl-realestatetype-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.RealEstateType();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-realestatetype-details is a template identifier for chosen model element
                templateName: '#tpl-realestatetype-details',
        ...
        getData: function () {
            return {
                idRealEstateType: $('#idRealEstateType').val(),
                realEstateTypeName: $('#realEstateTypeName').val(),
                realEstateTypeDescription: $('#realEstateTypeDescription').val()
            };
        }
    });
    new AppRouter();
