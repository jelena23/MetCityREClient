...
// Model for Country entity
    models.Country = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.country/",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idCountry');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.country/';
            }
    ...
    // Collection class for Country entities
    models.CountryCollection = Backbone.Collection.extend({
        model: models.Country,
        url: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.country/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
            ...
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        ...
                        // tpl-country-list-itemis template identifier for item
                        templateName: '#tpl-country-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.country = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.country,
                    // tpl-country-details is template identifier for chosen model element
                    templateName: '#tpl-country-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Country();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-country-details is a template identifier for chosen model element
                templateName: '#tpl-country-details',
