...
// Model for Municipality entity
    models.Municipality = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.municipality/",
        idAttribute: 'idMunicipality',
        defaults: {
            municipalityName: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idMunicipality');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.municipality/';
            }
    ...
    // Collection class for Municipality entities
    models.MunicipalityCollection = Backbone.Collection.extend({
        model: models.Municipality,
        url: "http://localhost:8080/MetCityRE/webresources/com.mycompany.metcityre.municipality/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.MunicipalityCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-municipality-list-itemis template identifier for item
                        templateName: '#tpl-municipality-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.municipality = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.municipality,
                    // tpl-municipality-details is template identifier for chosen model element
                    templateName: '#tpl-municipality-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Municipality();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-municipality-details is a template identifier for chosen model element
                templateName: '#tpl-municipality-details',
        ...
        getData: function () {
            return {
                idMunicipality: $('#idMunicipality').val(),
                municipalityName: $('#municipalityName').val()
            };
        }
    });
    new AppRouter();
